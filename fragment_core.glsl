...
void main(){
	...
	if ((vs_normal == vec3(0.f, 0.f, 0.f) && vs_texcoord==vec2(0.f,0.f)))
	fs_color = vec4(vs_color, 1.f);
	...
	if (vs_normal != vec3(0.f, 0.f, 0.f) && vs_texcoord==vec2(0.f,0.f))
	fs_color = vec4(vs_color, 1.f)*(vec4(ambientFinal, 1.f)+vec4(diffuseFinal, 1.f)+vec4(specularFinal, 1.f));

	if(vs_texcoord!= vec2(0.f,0.f) && vs_normal!=vec3(0.f, 0.f, 0.f))
	fs_color = texture(material.diffuseTex, vs_texcoord)*(vec4(ambientFinal, 1.f)+vec4(diffuseFinal, 1.f)+vec4(specularFinal, 1.f));
}
